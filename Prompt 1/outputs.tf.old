...
output "iam_role_arn" {
  ...
  value       = aws_iam_role.spacelift_worker.arn
}

output "launch_template_id" {
  description = "ID of the launch template for worker instances"
  value       = aws_launch_template.spacelift_worker.id
}

output "autoscaling_group_name" {
  description = "Name of the Auto Scaling Group"
  value       = aws_autoscaling_group.spacelift_worker.name
}

output "worker_instances_info" {
  description = "Information about how to check worker instances"
  value       = "Use 'aws ec2 describe-instances --filters Name=tag:Type,Values=spacelift-worker' to see worker instances"
}